...
def search_by_single_file(values, lang, window):
    ...
    if len(subtitles) == 0: # If finding movie with hash failed and list "subtitles" is empty so it length is 0
        ...
        if movie_name != None:
            ...
            link = opensubs.create_link(imdb=movie.imdb_id, hash=movie.file_hash, bytesize=movie.byte_size, query=f'{movie_name} {movie.excess} {movie.group}', language=lang) # Create a link to search for movie by its name and language
        try:
            subtitles = opensubs.request_subtitles(link)
